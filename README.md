...
[Try the demo](#-try-it-without-installing) ·
[Install](#-install) ·
[Feature inventory](docs/Features.md) ·
[Dev log](app/README.md) ·
...
> As of **2026-08-26**: weighing, masters, printing (a real managed print-template list and a
> single default-printer picker), reports/dashboard, backup/restore, the audit hash chain with
> public QR verification, offline licence activation, a Windows installer, remote access, an
...
> SQLite database, not just the mock.
>
> Real USB/IP/RTSP/ONVIF camera capture, ANPR, a visual print-template designer, anomaly
...
| | |
|---|---|
| 📱 **Android, from the same codebase** | `npx tauri android build` compiles and packages a real `.apk`/`.aab` for `aarch64-linux-android` today — a build that actually links and assembles (see `app/README.md` item 40). Not yet signed for the Play Store or run on a device |
| 🔐 **Offline licensing, no server** | A machine-bound request code, an Ed25519-signed activation code from an offline CLI (`tools/license-format`, `tools/licensegen`), verification baked into the binary — activation never touches a network. A 14-day trial gates new captures on expiry without ever blocking data already on disk |
| ⛓️ **Hash-chained audit trail** | Every document carries a content hash; the append-only `audit` table chains them together so a quietly-edited record breaks the chain and gets caught. A LAN-only verification server (opt-in Cloudflare Tunnel for public access) serves the page a ticket's QR code resolves to |
| 📤 **A real background outbox worker** | `useOutboxWorker.ts` polls every 30 seconds and retries `Pending`/backed-off-`Failed` rows across five channels — Email, SMS, Webhook, Tally-export, and outdoor display Board — not a one-shot "drain of one" at send time |
| 🎨 **Six themes, four text sizes** | Indicator/Graphite/Night shift/Paper/Daylight/High-contrast skins and a four-step text scale, all live-applied instantly, available even before an admin unlocks anything else |
| 🌐 **Genuinely bilingual** | English and a real, hand-translated Tamil pack (`app/src/i18n/packs/ta.ts`, ~290 keys) covering navigation, weighing, reports, dashboard, cameras, masters and most of settings — with a runtime language-pack uploader, falling back to English per missing key rather than breaking |
| 📊 **Dependency-free spreadsheet export** | `engines/export/xlsx.ts` is a hand-rolled OOXML writer — no library — alongside an RFC-4180 CSV writer with a UTF-8 BOM so Tamil and ₹ render correctly in Excel |
| 📦 **A legacy-data import path** | A documented JSON interchange format lets a site bring its v1/v2 history in as an idempotent, previewed bulk import — not a native `.mdb`/`.db` reader, but a real, working migration path |
...
release for each site. In BabuScales, **a customer's fields, screen layout, ticket design,
charging rules and numbering are all configuration** — onboarding a new site is an afternoon, not
a build.
...
Multi-copy DUPLICATE watermarking, mass print from Reports, and a real managed print-template
list (upload/paste HTML, save/select/preview/edit/delete) are built; a visual designer for custom
layouts, pre-printed-stationery alignment, and a true RAW/ESC-P spooler path (bypassing the OS
print dialog) are designed for but not built yet.
...
**→ Open the demo** — live once the Pages workflow has run once against `main`. Until then, open
`demo/BabuScales-demo.html` in any browser for the original one-file, no-build, no-database mock
...
| | |
|---|---|
| **Tauri v2 + Rust** | ~70MB RAM, sub-second start, one codebase for the future Android app — installer is ~200MB with the WebView2 runtime baked in for offline installs (see [Install](#-install), above) |
...
Chosen deliberately for **4GB machines**: the whole application uses less memory than a browser
tab.
...
balances and a durable submission queue are part of the core data model, with the **MiMaS**
client — blocked today on an integration spec that doesn't exist yet.

---

## 🙌 Credits

Built in an ongoing pairing session with
<img src="https://claude.ai/favicon.ico" alt="Claude Code" width="18" height="18" />
