...
pub fn list_masters(conn: &Connection, query: &MasterQuery) -> Result<Vec<MasterRow>, AppError> {
    ...
    qb.push_opt(&query.master_kind, "master_kind = :master_kind", ":master_kind", |v| {
        v.clone()
    });
    qb.push_opt(&query.is_active, "is_active = :is_active", ":is_active", |v| *v as i64);
    ...
    // substring search over master names. Without ESCAPE, `%`/`_` typed into
    // the search box act as SQL wildcards rather than literal characters — a
    // search for "A_1" would silently also match "AB1". `\` is escaped into
    // the pattern below alongside them so a literal backslash in the search
    // term doesn't itself get read as an escape introducer.
    qb.push_opt(
        &query.search,
    ...
    if let Some(after) = &query.after {
        ...
        qb.push_param(":after_name", after.name.clone());
        qb.push_param(":after_master_id", after.master_id.clone());
    }
    ...
    let rows = statement
        .query_map(qb.bindings().as_slice(), row_to_master)?
