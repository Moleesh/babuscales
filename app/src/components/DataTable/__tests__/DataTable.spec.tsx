...
describe("DataTable", () => {
    ...
    it("renders one header cell per column and one row per data row", () => {
        renderWithI18n(<DataTable columns={COLUMNS} rows={ROWS} getRowId={(row) => row.id} />);
    ...
    it("calls onRowClick with the clicked row when given", () => {
        ...
        renderWithI18n(<DataTable columns={COLUMNS} rows={ROWS} getRowId={(row) => row.id} onRowClick={onRowClick} />);
