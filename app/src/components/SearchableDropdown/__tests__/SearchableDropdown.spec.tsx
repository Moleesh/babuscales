...
describe("SearchableDropdown — keyboard navigation", () => {
    it("selects an option via keyboard alone: ArrowDown then Enter", () => {
        const onPick = vi.fn();
        const onChange = vi.fn();
        render(
            <SearchableDropdown value="Ganesh" onChange={onChange} onSearch={() => OPTIONS} onPick={onPick} />,
        );
        ...
        expect(onPick).toHaveBeenCalledWith(OPTIONS[0]);
        expect(onChange).toHaveBeenCalledWith(OPTIONS[0]?.Label);
        expect(screen.queryByRole("listbox")).toBeNull();
    });

    it("wraps ArrowUp from the top of the list to the last option", () => {
        const onPick = vi.fn();
        render(
            <SearchableDropdown value="Ganesh" onChange={vi.fn()} onSearch={() => OPTIONS} onPick={onPick} />,
        );
