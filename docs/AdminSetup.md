...
- MiMaS integration is blocked on an external specification that doesn't exist yet; not a bug, not
  in progress.
- Android: a real debug `.apk`/`.aab` now builds successfully (hardware-specific bits — printer
  enumeration, credential storage, the machine ID used for licensing — fall back to honest
  platform-appropriate behaviour there, the same way they already do for the browser/Pages demo
  build). Not yet done: signing for the Play Store or a real device/emulator install-and-run pass
  (none was available to test against) — this is a debug build proving the packaging pipeline, not
  a released one.
